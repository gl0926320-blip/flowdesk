  ...
  .update({
    ...
    (u: any) => u.can_access_estoque === true
      ...
      const totalServicos = relatedServices.length;
const propostas = relatedServices.filter((s: any) =>
  ["proposta_enviada", "aguardando_cliente", "proposta_validada"].includes(
    String(s.status || "")
  )
).length;

const andamento = relatedServices.filter((s: any) => s.status === "andamento").length;

const perdidos = relatedServices.filter((s: any) =>
  ["perdido", "cancelado", "recusado"].includes(String(s.status || ""))
).length;

const clientesUnicos = new Set(
  relatedServices
    .map((s: any) => String(s.cliente || s.nome_cliente || "").trim().toLowerCase())
    .filter(Boolean)
).size;
...
const lastActivity =
  relatedServices
    .map((s: any) => s.updated_at || s.created_at || s.data_orcamento || null)
    .filter(Boolean)
...
const ownerRow = ownersRows[0] || null;
const ownerAuth = ownerRow?.user_id ? authMap.get(ownerRow.user_id) : null;
const ownerLastLogin = ownerAuth?.last_sign_in_at || null;
...
const emptyCompanies = companyStatsRaw.filter((c: any) => c.canDelete).length;
...
const totalPerdidos = servicos.filter((s: any) =>
  ["perdido", "cancelado", "recusado"].includes(String(s.status || ""))
).length;

const totalPropostas = servicos.filter((s: any) =>
  ["proposta_enviada", "aguardando_cliente", "proposta_validada"].includes(
    String(s.status || "")
  )
).length;

const totalAndamento = servicos.filter((s: any) => s.status === "andamento").length;

const totalCampanhasAtivas = campaigns.filter((c: any) => c.is_active !== false).length;

const totalUsuariosComLogin = authUsers.filter((u: any) => u.last_sign_in_at).length;
const totalUsuariosSemLogin = authUsers.filter((u: any) => !u.last_sign_in_at).length;

const ultimosLogins = mergedUsersRaw
...
const empresasMaisAtivas = [...companyStatsRaw]
  .sort(
    (a: any, b: any) =>
...
const empresasComMaisLeads = [...companyStatsRaw]
  .sort((a: any, b: any) => Number(b.totalServicos || 0) - Number(a.totalServicos || 0))
  .slice(0, 6);
  ...
  <Card title="Logins">
    <Big>{totalUsuariosComLogin}</Big>
    <Small>{totalUsuariosSemLogin} nunca logaram</Small>
  </Card>
  ...
  <Card title="Leads">
    <Big>{totalLeads}</Big>
    <Small>{totalPropostas} propostas · {totalAndamento} andamento</Small>
  </Card>

  <Card title="Concluídos">
    <Big>{totalConcluidos}</Big>
    <Small>{totalPerdidos} perdidos/cancelados</Small>
  </Card>

  <Card title="Campanhas">
    <Big>{campaigns.length}</Big>
    <Small>{totalCampanhasAtivas} ativas</Small>
  </Card>
...
<section className="mb-8 grid grid-cols-1 gap-4 xl:grid-cols-3">
  <Panel title="Últimos logins">
    <div className="grid gap-3">
      {ultimosLogins.length === 0 ? (
        <EmptyText>Nenhum login encontrado ainda.</EmptyText>
      ) : (
        ultimosLogins.map((user: any) => (
  ...
  <Panel title="Empresas mais ativas">
    <div className="grid gap-3">
      {empresasMaisAtivas.map((item: any) => (
        <div key={item.id} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          <div className="font-semibold text-white">{item.name}</div>
          <div className="mt-1 text-xs text-white/55">
            Última atividade: {formatDate(item.lastActivity)}
          </div>
          <div className="mt-2 text-xs text-white/60">
            {item.totalServicos} leads/orçamentos · {money(item.receita)}
          </div>
        </div>
      ))}
    </div>
  ...
  <Panel title="Empresas com mais leads">
    <div className="grid gap-3">
  ...
  <div className="grid gap-2">
    <input
      ...
      list={`platform-users-${company.id}`}
      placeholder="Digite o e-mail do usuário para associar à empresa"
      required
    />

    <datalist id={`platform-users-${company.id}`}>
      {authUsers
        .map((user: any) => normalizeEmail(user.email))
        .filter(Boolean)
        .map((email: string) => (
          <option key={`${company.id}-${email}`} value={email} />
        ))}
    </datalist>

    <p className="text-[11px] text-white/45">
      Digite o e-mail do usuário já cadastrado na plataforma. Se o e-mail ainda não existir no Auth,
      o sistema cria um vínculo pendente para esse e-mail.
    </p>
  </div>
